...
export default {
  ...
  methods: {
    ...
    unloadListener() {
      ...
      sessionStorage.setItem(process.env.VUE_APP_TABS_KEY, JSON.stringify(tabs))
    },
    createPage(route) {
      return {
     ...
     */
    loadCachedTabs() {
      const cachedTabsStr = sessionStorage.getItem(process.env.VUE_APP_TABS_KEY)
      if (cachedTabsStr) {
        try {
          ...
          sessionStorage.removeItem(process.env.VUE_APP_TABS_KEY)
        }
      }
    },
